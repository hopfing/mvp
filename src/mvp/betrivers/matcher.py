...
class BetRiversOddsMatcher(BaseJob):
    ...
    def _load_aliases(self) -> dict[str, str]:
        """Load alias YAML (normalized BR name -> player_id)."""
    ...
    def _resolve_id(self, name: str) -> str | None:
        """Resolve a BR player name to our player_id."""
    ...
    def get_latest_odds(self) -> pl.DataFrame:
        """Read BR odds parquet, deduplicated to latest per event+player."""
    ...
    def match(self, predictions: pl.DataFrame) -> OddsMatchResult:
        ...
        for eid, br_rows in br_events.items():
            ...
            for br_row in br_rows[:2]:
                pid = self._resolve_id(br_row["player_name"])
                if pid is None:
                    unmatched_names.add(br_row["player_name"])
                else:
                    ids_and_odds.append((pid, br_row["odds"]))
        ...
        logger.info(
            "Odds matching: %d/%d BR events matched to %d predictions",
            matched, total_events, total_preds,
        )
        if unmatched_names:
            logger.info(
                "Unmatched BR names (%d): %s",
                len(unmatched_names),
