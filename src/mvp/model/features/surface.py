...
def surface_win_pct(days: int | None = None) -> pl.Expr:
    ...
    """
    if days is None:
        return cumulative_mean("won", group_by=_SURFACE_GROUP)
    return rolling_mean("won", days=days, group_by=_SURFACE_GROUP)
...
def surface_hold_pct(days: int | None = None) -> pl.Expr:
    ...
    return ratio_feature(holds, "svc_games_played", days, group_by=_SURFACE_GROUP)
